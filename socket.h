...
typedef struct {
	...
	int client_accepted;
	// File descriptor to a lock file that will be filled in once the socket is binded.
	// The port of the listening socket is written to it. The client is
...
int
socket_send(SocketListener* socket_listener, char* msg, size_t msg_size);
