...
int setup_nonblocking_listener(SocketListener *socket_listener) {
  ...
  if (socket_listener->listening_socket_fd == -1) {
    dprintf(socket_listener->log_file,
            "[log-tabbed] setup_nonblocking_listener can't open socket, "
            "socket() == -1\n");
    return -1;
  }
  /*
  ...
  ret = bind(socket_listener->listening_socket_fd,
             (const struct sockaddr *)&addr, sizeof(struct sockaddr_in));
  if (ret == -1) {
    dprintf(socket_listener->log_file,
            "[error-tabbed] setup_nonblocking_listener : socket bind(%u, %s, "
            "%u) = -1\n",
            socket_listener->listening_socket_fd, inet_ntoa(addr.sin_addr),
            htons(addr.sin_port));
    return -1;
  }
  {
    ...
    if (getsockname(socket_listener->listening_socket_fd,
      ...
      dprintf(socket_listener->log_file,
              "[error-tabbed] setup_nonblocking_listener : can't get assigned "
              "port, getsockname(%u)\n",
              socket_listener->listening_socket_fd);
      return -1;
    } else {
      socket_listener->socket_port = ntohs(given_addr.sin_port);
      /* dprintf(socket_listener->log_file, "[log-tabbed]
       * setup_nonblocking_listener : assigned socket port is `%u`\n", */
      /* 		socket_listener->socket_port); */
      char buf[10];
      if (snprintf(buf, 9, "%u", socket_listener->socket_port) >= 9) {
        // Impossible case ?
        dprintf(
            socket_listener->log_file,
            "[error-tabbed] setup_nonblocking_listener, buffer too little\n");
        return -1;
      }
      if (write(socket_listener->lock_port_fd, buf, 9) == -1) {
        dprintf(socket_listener->log_file,
                "[error-tabbed] setup_nonblocking_listener : can't write "
                "assigned port to lock file, write(%u, %s) == -1\n",
                socket_listener->lock_port_fd, buf);
        return -1;
      } else {
        // dprintf(socket_listener->log_file, "[log-tabbed]
        // setup_nonblocking_listener : port written to lock\n");
      }
    }
  }
  ...
  if (ret == -1) {
    dprintf(socket_listener->log_file,
            "[error-tabbed] setup_nonblocking_listener : listen(%u, 2)\n",
            socket_listener->listening_socket_fd);
    return -1;
  }
...
int loop_listen_nonblocking(SocketListener *socket_listener, char *buffer,
  ...
  if (!socket_listener->client_accepted) {
    ...
    socket_listener->poll_fds[0].fd =
        accept(socket_listener->listening_socket_fd, NULL, NULL);
    ...
    case -1:
      dprintf(socket_listener->log_file,
              "[error-tabbed] setup_nonblocking_listener : accept(%u) == -1",
              socket_listener->listening_socket_fd);
      return -1;
    }
  ...
  if (socket_listener->open_fds_count > 0) {
    ...
    case -1:
      dprintf(socket_listener->log_file,
              "[error-tabbed] loop_listen_nonblocking : poll(fd, %lums) = -1\n",
              timeout);
      return -1;
    case 0:
      // Timed out
    ...
    /* dprintf(socket_listener->log_file, "[log-tabbed] loop_listen_nonblocking
     * : poll ready: %d\n", ready); */
    ...
    if (socket_listener->poll_fds[0].revents != 0) {
      ...
      /* { */
      /* 		dprintf(socket_listener->log_file, "[log-tabbed]
       * loop_listen_nonblocking fd=%d; events: %s%s%s%s%s%s%s%s\n",
       * socket_listener->poll_fds[0].fd, */
      /* 			/1* (socket_listener->poll_fds[0].revents &
       * POLLIN)  ? "POLLIN "  : "", *1/ */
      /* 			(socket_listener->poll_fds[0].revents &
      ...
      if (socket_listener->poll_fds[0].revents & POLLIN) {
        ...
        if (s == -1) {
          // When read error, exit and wait for another message (by recalling
          // the loop)
          dprintf(socket_listener->log_file,
                  "[error-tabbed] loop_non_blocking recv(%u) == -1\n",
                  socket_listener->poll_fds[0].fd);
          return 0;
        }
        if (s > 0) {
          ...
          /* 		buf, buffer, s+1); */
          dprintf(socket_listener->log_file,
                  "[log-tabbed] loop_non_blocking : received %zd bytes: %.*s\n",
                  s, (int)s, buffer);
          return 2;
        } else if (s == 0) {
          if (socket_listener->empty_pollin_received_count == 0) {
            socket_listener->first_empty_pollin_received_time =
                get_posix_time();
          }
          ...
          if (socket_listener->empty_pollin_received_count > 10 &&
            ...
            if (close(socket_listener->poll_fds[0].fd) == -1) {
              dprintf(socket_listener->log_file,
                      "[error-tabbed] loop_listen_nonblocking : can't close fd "
                      "`%u`\n",
                      socket_listener->poll_fds[0].fd);
            } else {
              /* dprintf(socket_listener->log_file, "[log-tabbed]
               * loop_listen_nonblocking : empty POLLINS, closed fd %d\n",
               * socket_listener->poll_fds[0].fd); */
          ...
          dprintf(socket_listener->log_file,
                  "[error-tabbed] loop_listen_nonblocking : read failed\n");
        }
        /* else { */
        /* 	dprintf(socket_listener->log_file, "[log-tabbed]
         * loop_listen_nonblocking closing fd %d\n",
         * socket_listener->poll_fds[0].fd); */
        /* 	if (close(socket_listener->poll_fds[0].fd) == -1) { */
        /* 		dprintf(socket_listener->log_file, "[error-tabbed]
         * loop_listen_nonblocking : can't close fd `%u`\n",
         * socket_listener->poll_fds[0].fd); */
        /* 	} */
        /* 	socket_listener->open_fds_count--; */
        /* 	return 0; */
      ...
      if (socket_listener->poll_fds[0].revents & POLLNVAL) {
        socket_listener->open_fds_count--;
        /* dprintf(socket_listener->log_file, "[log-tabbed]
         * loop_listen_nonblocking : POLLNVAL, client closed fd %d\n",
         * socket_listener->poll_fds[0].fd); */
      }
      if (socket_listener->poll_fds[0].revents & POLLHUP ||
        ...
        if (close(socket_listener->poll_fds[0].fd) == -1) {
          ...
          dprintf(
              socket_listener->log_file,
              "[error-tabbed] loop_listen_nonblocking : can't close fd `%u`\n",
              socket_listener->poll_fds[0].fd);
        } else {
          /* dprintf(socket_listener->log_file, "[log-tabbed]
           * loop_listen_nonblocking : POLLHUP or POLLERR, closed fd %d\n",
           * socket_listener->poll_fds[0].fd); */
          socket_listener->open_fds_count--;
        }
        return 0;
...
void loop_send(SocketListener *socket_listener) {
  if (QUEUE_SIZE(socket_listener->messages_queue) > 0) {
    ...
    /* dprintf(socket_listener->log_file, "[log-tabbed] loop_send : sending
     * `%s`:%ld\n to fd `%d`", */
    /* 		message->content, strlen(message->content),
     * socket_listener->poll_fds[0].fd); */
    ...
    if (send(socket_listener->poll_fds[0].fd, message->content,
      ...
      dprintf(socket_listener->log_file,
              "[error-tabbed] socket_send : send(%u, %s) == -1\n",
...
int socket_send(SocketListener *socket_listener, char *message_to_send,
                size_t msg_size) {
  if (socket_listener->open_fds_count <= 0) {
    /* dprintf(socket_listener->log_file, "[error-tabbed] socket_send : can't
     * send message, no fds open\n"); */
    return -1;
  } else {
    const size_t sizeof_msg_content = sizeof(((struct msg *)0)->content);
    ...
    /* dprintf(socket_listener->log_file, "[log-tabbed] socket_send : message
     * `%s` pushed\n", */
    /* message->content); */
  }
  return 0;
...
int make_socket_listener(SocketListener *socket_listener,
  ...
  if (socket_listener->lock_port_fd == -1) {
    dprintf(socket_listener->log_file,
            "[error-tabbed] make_socket_listener : mkstemp(%s) == -1\n",
...
int wait_for_socket_port_lock(SocketListener *socket_listener, int delay) {
  ...
  struct timespec delay_time = {.tv_sec = 0, .tv_nsec = delay * 1000000};
  ...
  while (statbuf.st_size == 0) {
    if (fstat(socket_listener->lock_port_fd, &statbuf) == -1) {
      dprintf(socket_listener->log_file,
              "[error-tabbed] wait_for_socket_port_lock : fstat(%u) == -1\n",
  ...
  /* dprintf(socket_listener->log_file, "[log-tabbed] wait_for_socket_port_lock
   * : server filled lock port file\n"); */
  // Reading the port number
  char port_buffer[12];
  ssize_t read_size = read(socket_listener->lock_port_fd, port_buffer, 11);
  if (read_size == -1) {
    dprintf(socket_listener->log_file,
            "[error-tabbed] wait_for_socket_port_lock : lock port file can't "
            "be read, read(%u) == -1\n",
            socket_listener->lock_port_fd);
    ...
    if (port == ULONG_MAX) {
      dprintf(socket_listener->log_file,
              "[error-tabbed] wait_for_socket_port_lock : invalid socket port "
              "in lock file : strtoul(%s, 10) == ERROR\n",
              port_buffer);
      return -1;
    } else {
      return port;
